(function (root, factory) {
        ...
        initialize: function () {
            ...
            _converse.api.listen.on('getToolbarButtons', function(toolbar_el, buttons)
            {
                ...
                if (contact?.get("show") == "offline" && window.WebPushLib.selfSecret)
                {
                    ...
                    buttons.push(html`
                        <button data-jid="${jid}" class="plugin-offline-reply" title="${offline_reply_label}" @click=${performClick}/>
                            <svg style="width:18px; height:18px; ${color}" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g><path d="M 27.91,11.79c 0.176,0.792, 0.278,1.606, 0.278,2.442c0,6.456-5.48,11.838-12.696,12.994 c 1.526,0.616, 3.236,0.976, 5.058,0.976c 1.718,0, 3.34-0.316, 4.804-0.866c 1.854,0.632, 3.878,0.796, 5.552,0.796 c-0.87-1.044-1.474-2.068-1.906-2.968C 30.856,23.508, 32,21.314, 32,18.898C 32,16.042, 30.406,13.496, 27.91,11.79zM 25.188,14.232C 25.188,8.582, 19.55,4, 12.594,4S0,8.582,0,14.232c0,2.948, 1.542,5.596, 3.998,7.464 c-0.462,1.192-1.244,2.75-2.57,4.344c 2.516,0, 5.756-0.33, 8.25-1.862c 0.938,0.182, 1.912,0.288, 2.916,0.288 C 19.55,24.464, 25.188,19.884, 25.188,14.232z"></path></g></svg>
                        </button>
                    `);
                }
    ...
    function subscribeOfflineReply(jid)
    {
        ...
        _converse.connection.sendIQ(stanza, function(result)
        {
            console.debug("subscribeOfflineReply ok", result);

        }, function(error){
            console.error("subscribeOfflineReply", error);
        });
    }
    ...
    function fetchOfflineReply(jid)
    {
        ...
        _converse.connection.sendIQ(stanza, function(result)
        {
            ...
            console.error("fetchOfflineReply", error);
        });
    }
